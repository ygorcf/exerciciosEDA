...
RegNome * obtemFimLista(RegNome *r);
...
int insereRegistroLista(RegNome *nomeEncontrado, RegNome *pivo){
    ...
    *q = *pivo;
    if(nomeEncontrado != NULL){
        ...
        if(q->prox) q->prox->ante = q;
        q->ante = nomeEncontrado;
    }else{
        q->prox = NULL;
        q->ante = NULL;
        nomeEncontrado = q;
...
int excluiRegistroLista(RegNome *nomeEncontrado, RegNome *pivo){
    if(nomeEncontrado != NULL && nomeEncontrado->prox != NULL){
        RegNome *q = nomeEncontrado->prox;
        nomeEncontrado->prox = nomeEncontrado->prox->prox;
        nomeEncontrado->prox->ante = nomeEncontrado;
        free(q);
        q = NULL;
        if(pivo != NULL) return 1;
    }
    return 0;
...
//
int listaInversa(SuperReg *admRegs){
  RegNome *r = admRegs->fimLista;
	while(r != NULL){
    printaNomeRegistro(r, NULL);
	  r = r->ante;
	}
	return 0;
}
...
int insere(SuperReg *admRegs){
    ...
    if(toupper(getch()) == 'S')
        if(admRegs->inicioLista == NULL){ // Nenhum
            ...
            if(admRegs->inicioLista != NULL){
                admRegs->fimLista = (RegNome *)(malloc(sizeof(RegNome)));
                if(admRegs->fimLista != NULL){
                    *admRegs->inicioLista = pivo;
                    *admRegs->fimLista = pivo;
                }else{
...
int criaBackup(SuperReg *admRegs, SuperReg *backupAdmRegs){
	...
	if(admRegs->inicioLista != NULL){
      ...
      if(backupAdmRegs->inicioLista != NULL){
        ...
        backupAdmRegs->fimLista = admRegs->fimLista;
      }
    }else{
  	  printCabInfer("Ocorreu um erro na memoria");
      ret = 1;
...
void recuperaBackup(SuperReg *admRegs, SuperReg *backupAdmRegs){
  freeLista(admRegs->inicioLista);
	...
	admRegs->fimLista = backupAdmRegs->fimLista;
